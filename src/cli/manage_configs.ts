...
export default async function manage_configs(paths_are_configured: boolean) {
    ...
    const cfg_type_selection = await prompts({
        type: 'select',
        name: 'selected_cfg_type',
    ...
    if (cfg_type_selection.selected_cfg_type === "paths-cfg")
        await manage_path_config(paths_are_configured);

    else if (cfg_type_selection.selected_cfg_type === "record-cfg")
